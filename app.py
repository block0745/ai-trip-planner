...
# Define CrewAI Agents with Groq
def create_agents(groq_api_key):
    # Initialize Groq LLM
    groq_llm = LLM(
    ...
    city_expert = Agent(
        ...
        backstory='You are an experienced travel researcher with deep knowledge of cities worldwide. '
    ...
    itinerary_planner = Agent(
        ...
        backstory='You are a professional travel planner with years of experience creating customized trips. '
...
# Generate trip plan using CrewAI with Groq
def generate_trip_plan(groq_api_key, origin, destination, start_date, end_date, interests, budget, travel_style):
    try:
        # Create agents
        city_expert, itinerary_planner = create_agents(groq_api_key)
        
        # Create tasks
        tasks = create_tasks(city_expert, itinerary_planner, origin, destination, 
...
st.markdown("### Plan Your Perfect Trip with CrewAI & Groq")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### About")
    st.info("This app uses CrewAI agents powered by Groq's fast LLM inference to create personalized trip plans.")
    
    st.markdown("**Why Groq?**")
    st.markdown("- ⚡ Ultra-fast inference")
    st.markdown("- 🆓 Free API access")
...
with col_btn2:
    ...
    if generate_button:
        ...
        with st.status("🤖 AI Agents are planning your trip...", expanded=True) as status:
            ...
            st.write("⚡ Using Groq's ultra-fast LLM...")
            
            # Generate the trip plan
            result = generate_trip_plan(
                groq_api_key, origin, destination, start_date, end_date, 
...
if st.session_state.trip_plan:
    ...
    # Display in an expandable container for better readability
    with st.container():
...
<div style='text-align: center; color: gray;'>
    <p>⚡ Built with Streamlit, CrewAI & Groq | Powered by Ultra-Fast AI Agents</p>
</div>
